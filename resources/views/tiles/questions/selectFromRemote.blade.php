...
<div class="form-group {{ $errors->has($field) ? 'has-error' : '' }}">
    ...
    <select id="{{$field}}_inputDiv" {{$isRequired}} class="form-control" name="{{$field}}" ng-model="{{$model}}.{{$field}}">
        ...
        <option ng-repeat="remoteRow in {{$remoteField}}" value="<% remoteRow.{{$remoteIdentifer}} %>" ng-selected="{{$model}}.{{$field}} == remoteRow.{{$remoteIdentifer}}"><%remoteRow.{{$remoteDisplayValue}}%></option>
    </select>
